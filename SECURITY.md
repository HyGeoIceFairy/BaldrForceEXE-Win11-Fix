...
7. 短暂暂停游戏主线程及当前线程快照中的辅助线程，再次核对目标字节后，把资源目录名的两个字节由 `Se` 改为 `Sx`。
8. 窗口化与无边框模式会调整两处 DirectDraw 激活状态分支，避免 dgVoodoo 托管的桌面窗口在最小化、恢复或切换焦点时错误释放并重建显示表面。
9. 窗口模式会分配一小段仅供目标游戏执行的 32 位坐标换算代码，并把三个已验证的鼠标坐标调用点重定向到该代码，使缩放后的客户区坐标与游戏固定的 640×480 逻辑坐标一致。启动器只在所有固定签名都匹配时写入，写入失败会回滚或终止仍处于暂停状态的游戏进程。
10. 完成内存补丁后恢复游戏线程，对窗口化或无边框模式调整游戏窗口，然后退出。
